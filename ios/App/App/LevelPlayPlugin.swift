...
// Keep the existing JavaScript plugin name while using the LevelPlay flow that
// is already working in the TrimSwipe app.
@objc(UnityAdsPlugin)
public final class UnityAdsPlugin: CAPPlugin, CAPBridgedPlugin, LPMRewardedAdDelegate, LPMInterstitialAdDelegate {
    ...
    @objc func initializeAds(_ call: CAPPluginCall) {
        guard let appKey = call.getString("gameId"), !appKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
        ...
        let adapterDebug = call.getBool("testMode", false)

        DispatchQueue.main.async {
            self.log("initialize start appKey=\(appKey) adapterDebug=\(adapterDebug)")
            if adapterDebug { LevelPlay.setAdaptersDebug(true) }
            ...
            LevelPlay.initWith(request) { [weak self] _, error in
                DispatchQueue.main.async {
                    ...
                    if let error {
                        ...
                        calls.forEach { $0.reject("LevelPlay initialization failed: \(error.localizedDescription)", "LEVELPLAY_INIT_FAILED") }
                        return
                    }
    ...
    @objc func loadInterstitial(_ call: CAPPluginCall) {
        ...
        DispatchQueue.main.async {
            ...
            if ad.isAdReady() { call.resolve(["loaded": true]); return }
    ...
    @objc func showInterstitial(_ call: CAPPluginCall) {
        ...
        DispatchQueue.main.async {
            ...
            guard self.pendingInterstitialShowCall == nil else { call.reject("LevelPlay interstitial is already showing"); return }
            guard let ad = self.interstitialAds[adUnitId], ad.isAdReady() else { call.reject("LevelPlay interstitial is not ready"); return }
            guard let viewController = self.activeViewController() else { call.reject("Could not find a view controller for LevelPlay"); return }
    ...
    @objc func showRewarded(_ call: CAPPluginCall) {
        ...
        DispatchQueue.main.async {
            ...
            guard self.pendingRewardedShowCall == nil else { call.reject("LevelPlay rewarded ad is already showing"); return }
            guard let ad = self.rewardedAds[adUnitId], ad.isAdReady() else { call.reject("LevelPlay rewarded ad is not ready"); return }
            guard let viewController = self.activeViewController() else { call.reject("Could not find a view controller for LevelPlay"); return }
    ...
    public func didFailToDisplayAd(with adInfo: LPMAdInfo, error: Error) {
        if rewardedAds[adInfo.adUnitId] != nil {
            ...
            pendingRewardedShowCall?.reject("LevelPlay rewarded display failed: \(error.localizedDescription)", "LEVELPLAY_REWARDED_DISPLAY_FAILED")
            ...
            pendingInterstitialShowCall?.reject("LevelPlay interstitial display failed: \(error.localizedDescription)", "LEVELPLAY_INTERSTITIAL_DISPLAY_FAILED")
            pendingInterstitialShowCall = nil
        }
    }

    public func didClickAd(with adInfo: LPMAdInfo) { log("ad callback didClickAd adUnitId=\(adInfo.adUnitId)") }
    ...
    public func didCloseAd(with adInfo: LPMAdInfo) {
        if rewardedAds[adInfo.adUnitId] != nil {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                self.pendingRewardedShowCall?.resolve(["completed": self.rewardedEarned, "rewarded": self.rewardedEarned])
    ...
    private func ensureInitialized(_ call: CAPPluginCall) -> Bool {
        guard initialized else { call.reject("LevelPlay is not initialized"); return false }
        return true
    }

    private func getAdUnitId(_ call: CAPPluginCall) -> String? {
        guard let adUnitId = call.getString("placementId"), !adUnitId.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
