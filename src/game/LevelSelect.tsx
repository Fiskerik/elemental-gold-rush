...
const CAMPAIGN_MAP_COLUMNS = [0.16, 0.39, 0.61, 0.84] as const;
...
export function LevelSelect({
  ...
  const nodes = useMemo(
    () =>
      LEVELS.map((lvl, i) => {
        const row = Math.floor(i / CAMPAIGN_MAP_COLUMNS.length);
        const colInRow = i % CAMPAIGN_MAP_COLUMNS.length;
        const colIdx = row % 2 === 0
          ? colInRow
          : CAMPAIGN_MAP_COLUMNS.length - 1 - colInRow;
        return {
          lvl,
          x: CAMPAIGN_MAP_COLUMNS[colIdx] * MAP_W,
          y: 58 + row * ROW_H,
        };
      }),
    [MAP_W, ROW_H],
  );
  const mapRows = Math.ceil(LEVELS.length / CAMPAIGN_MAP_COLUMNS.length);
