...
function storeReferralCode(code: string): void {
  try {
    ...
    // Referral redemption remains available for the current session.
  }
}
...
export async function settleCompletedReferral(
  ...
  const completed = await referralRequest<{ awarded?: boolean }>("/completeReferral", {
    playerId,
  });
  const claimed = await referralRequest<{ coins?: number }>("/claimReferralRewards", {
    playerId,
  });
  return {
